...
- 存储位置：本插件凭证在 `~/.dsh/.credentials.yaml` 的 `records.dsh-openai-subscription/chatgpt`，`kind: grant`，payload 字段：`provider / loginMethod / accountId / access / refresh / expires / obtainedAt / refreshedAt`；模型镜像在同一文件的 `records.llm-pi-ai/openai-codex`，按 pi-ai 凭证形状存储
- 消费方读取方式（宿主插件内）：
