...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function parseStatus(value: unknown): StatusInfo {
      ...
      if (typeof raw.loginMethod === 'string') status.loginMethod = raw.loginMethod;
    ...
    function Section(props: SectionProps): ReactElement {
      ...
      return el('div', { className: 'oasub-wrap' },
        el('div', { className: 'oasub-title', role: 'heading', 'aria-level': 2 }, 'OpenAI 订阅登录'),
        ...
        !ready ? el('div', { className: 'oasub-err', role: 'alert' }, '未找到 DSH 的 pi 依赖（@earendil-works/pi-ai），无法使用订阅登录。') : null,
        info === null
        ...
        phase === 'done' && result === 'logged-out' ? el('div', { className: 'oasub-ok', role: 'status', 'aria-live': 'polite' }, '已退出 OpenAI 订阅账号。') : null,
        info === null ? null : el('div', { className: 'oasub-row' },
          configured
            ...
            : el('button', { type: 'button', className: 'oasub-btn primary', disabled: busy || !ready, onClick: () => start('device_code') }, authBusy ? '登录中…' : '使用 OpenAI 账号登录'),
