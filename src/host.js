...
class OpenAISubscriptionController extends TypertRemoteService {
    ...
    credentials() {
        if (this._credentials === undefined)
            this._credentials = this.ctx.get('credentials');
        return this._credentials;
    }
    shell() {
        if (this._shell === undefined)
            this._shell = this.ctx.get('shell');
        return this._shell;
    }
    timer() {
        if (this._timer === undefined)
            this._timer = this.ctx.get('timer');
        return this._timer;
    }
    authorization() {
        if (this._authorization === undefined)
            this._authorization = this.ctx.get('authorization');
        return this._authorization;
    }
    ensureAuthorizationFlow() {
        const authorization = this.authorization();
        if (this._flowRegistered || authorization === undefined)
            return;
    ...
    async runRefresh(control, notify) {
        ...
        await credentials.modifyRecord(KEY, async () => ({
            ...
            payload: {
                ...
                accountId: typeof next.accountId === 'string' ? next.accountId : (payload.accountId ?? null),
                ...
                expires: typeof next.expires === 'number' ? next.expires : (payload.expires ?? null),
